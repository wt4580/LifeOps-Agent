...
env_file_name = ".env"
env_path=ENV_DIR / env_file_name
...
class Settings(BaseSettings):
    ...
    database_url: str = f"sqlite:///{(Path(__file__).resolve().parent.parent.parent.parent / 'data' / 'lifeops.db').as_posix()}"
    # 知识库索引单独落到独立 SQLite，避免和业务表/检查点耦合。
    knowledge_database_url: str = Field(
        default=f"sqlite:///{(Path(__file__).resolve().parent.parent.parent.parent / 'data' / 'knowledge_base.db').as_posix()}",
        alias="KNOWLEDGE_DATABASE_URL",
    )
    # LangGraph checkpointer 的 SQLite 文件路径。
    checkpointer_db_path: str = Field(
        default=str((Path(__file__).resolve().parent.parent.parent.parent / 'data' / 'checkpointer.db').as_posix()),
        alias="CHECKPOINTER_DB_PATH",
    )
    ...
    amap_weather_key: str = Field(default="", alias="AMAP_WEATHER_KEY")
    # 当用户未提供城市且 profile 中也没有 default_city 时使用的兜底城市
    default_weather_city: str = Field(default="", alias="DEFAULT_WEATHER_CITY")
    # 天气查询结果内存缓存秒数（节省免费额度）
